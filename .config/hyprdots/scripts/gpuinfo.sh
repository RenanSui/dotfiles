...
generate_json() {
  ...
  # Se gpu_load existir (AMD), usa ele.
  # Se não, usa utilization (NVIDIA / general_query)
  local usage="${gpu_load:-$utilization}"

  # Começa o JSON
  local json="{"
  json+="\"text\":\"${thermo} ${temperature:-0}°C\","
  json+="\"tooltip\":\"${primary_gpu}"

  # Tooltip base
  [[ -n "$temperature" ]] && json+="\n${thermo} Temperature: ${temperature}°C ${emoji}"
  [[ -n "$usage" ]] && json+="\n${speedo} Utilization: ${usage}%"
  [[ -n "$current_clock_speed" && -n "$max_clock_speed" ]] && \
      json+="\n Clock Speed: ${current_clock_speed}/${max_clock_speed} MHz"
  [[ -n "$core_clock" ]] && \
      json+="\n Core Clock: ${core_clock} MHz"
  [[ -n "$power_usage" && -n "$power_limit" ]] && \
      json+="\n󱪉 Power Usage: ${power_usage}/${power_limit} W"
  [[ -n "$power_usage" && -z "$power_limit" ]] && \
      json+="\n󱪉 Power Usage: ${power_usage} W"
  [[ -n "$power_discharge" && "$power_discharge" != "0" ]] && \
      json+="\n Power Discharge: ${power_discharge} W"

  json+="\","

  # 🔥 CAMPOS EXTRAS PRO WAYBAR
  [[ -n "$usage" ]] && json+="\"percentage\":${usage},"
  [[ -n "$temperature" ]] && json+="\"temperature\":${temperature},"
  [[ -n "$current_clock_speed" ]] && json+="\"current_clock\":${current_clock_speed},"
  [[ -n "$max_clock_speed" ]] && json+="\"max_clock\":${max_clock_speed},"
  [[ -n "$core_clock" ]] && json+="\"core_clock\":${core_clock},"
  [[ -n "$power_usage" ]] && json+="\"power_usage\":${power_usage},"
  [[ -n "$power_limit" ]] && json+="\"power_limit\":${power_limit},"
  [[ -n "$power_discharge" ]] && json+="\"power_discharge\":${power_discharge},"
...
amd_GPU() { #? Funtion to query amd GPU
  ...
  amd_output=$(${ScrDir}/venv/bin/python ${ScrDir}/amdgpu.py)
